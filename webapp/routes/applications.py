...
async def apply_submit(request: Request, property_id: int):
    ...
    async with get_session() as session:
        ...
        # Parse income
        income = None
        income_str = form.get("income", "").strip()
        if income_str:
            try:
        ...
        # Parse voucher amount
        voucher_amount = None
        has_voucher = form.get("has_section8_voucher") == "on"
        if has_voucher:
            va_str = form.get("voucher_amount", "").strip()
            if va_str:
                try:
                    voucher_amount = float(va_str.replace(",", "").replace("$", ""))
                except ValueError:
                    pass
        ...
        # Parse occupants
        num_occupants = None
        occ_str = form.get("num_occupants", "").strip()
        if occ_str:
            try:
                num_occupants = int(occ_str)
            except ValueError:
                pass
        ...
        application = TenantApplication(
            ...
            applicant_current_address=form.get("current_address", "").strip(),
